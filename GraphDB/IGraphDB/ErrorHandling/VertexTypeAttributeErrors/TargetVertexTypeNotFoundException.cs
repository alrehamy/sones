...
namespace sones.GraphDB.ErrorHandling
{
    ...
    public class TargetVertexTypeNotFoundException : AGraphDBVertexAttributeException
    {
												 ...
												 Exception innerException = null) : base(innerException)
        {
            ...
            this.Edges = myEdges;
            _msg = string.Format("The outgoing edges ({0}) on vertex type {1} does point to a not existing target type {2}.", String.Join(",", myEdges.ToArray()), myPredefinition.TypeName, myTargetVertexTypeName);
        }
    }
}
